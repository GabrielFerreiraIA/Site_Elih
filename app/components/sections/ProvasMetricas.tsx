...
export default function ProvasMetricas() {
  ...
  return (
    <section
      id="provas"
      className="relative bg-navy-950 border-b border-white/10 overflow-hidden"
    >
      {/* ── Background layers ──────────────────────────────────────────── */}
      <div aria-hidden className="absolute inset-0 -z-20 bg-navy-950" />
      <div
        aria-hidden
        className="absolute inset-0 -z-20 bg-[radial-gradient(ellipse_55%_45%_at_50%_-5%,rgba(37,99,235,0.12),transparent_55%),radial-gradient(ellipse_40%_40%_at_85%_30%,rgba(1,18,70,0.4),transparent_60%)]"
      />
      {/* bottom light beam */}
      <div
        aria-hidden
        className="absolute bottom-0 left-1/2 -translate-x-1/2 h-px w-[55%] bg-gradient-to-r from-transparent via-white/20 to-transparent"
      />
      <div
        aria-hidden
        className="absolute bottom-0 left-1/2 -translate-x-1/2 h-16 w-[40%] bg-[radial-gradient(ellipse_at_bottom,rgba(37,130,244,0.15),transparent_70%)] blur-md"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-16">
        ...
        >
          <div className="flex items-center gap-5">
            ...
            <div className="relative flex-1 h-px bg-gradient-to-r from-white/10 to-transparent">
              <motion.span
                aria-hidden
                className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-white shadow-[0_0_12px_4px_rgba(242,245,255,0.2)]"
          ...
          <h2 className="mt-4 font-display text-3xl sm:text-4xl lg:text-[2.25rem] lg:leading-tight font-semibold tracking-tight text-white text-balance">
          ...
          <div className="lg:col-span-7">
            ...
            <motion.div
              ...
              className="group relative rounded-2xl overflow-hidden
                         glass-dark border border-white/10 shadow-[var(--shadow-lg)]"
            >
              <EdgeFrame radius="rounded-2xl" />
              {/* corner sheen */}
              <div
                aria-hidden
                className="absolute -top-px -left-px w-40 h-40 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.04),transparent_70%)] rounded-2xl"
              />
              ...
              <div className="relative p-6 sm:p-7 lg:p-8">
                ...
                <span
                  aria-hidden
                  className="block font-sans leading-[0.5] text-white/15 select-none text-5xl sm:text-6xl mb-0.5"
                >
                  &ldquo;
                </span>
                ...
                <div className="min-h-[110px] sm:min-h-[115px]">
                  <AnimatePresence mode="wait" custom={dir}>
                    ...
                    >
                      <blockquote className="font-sans text-lg sm:text-xl lg:text-[22px] leading-relaxed text-white text-balance">
                        {current.quote}
                      </blockquote>
                      <figcaption className="mt-5 flex items-center gap-3">
                        <span aria-hidden className="w-6 h-px bg-gradient-to-r from-white/30 to-transparent" />
                        <span className="font-display uppercase tracking-[0.14em] text-[10px] sm:text-xs text-white/60">
                          {current.author}
                      ...
                      >
                        <span
                          className={`block h-1 rounded-full transition-all duration-500 ${
                            i === active
                              ? "w-6 bg-gradient-to-r from-white to-accent-100 shadow-[0_0_6px_rgba(242,245,255,0.4)]"
                              : "w-1 bg-white/20 group-hover/dot:bg-white/50"
                          }`}
                        />
                      </button>
                    ))}
              ...
              {current.metrics.map((m) => (
                <motion.div
                  ...
                  className="group relative rounded-xl overflow-hidden p-4 sm:p-5
                             glass-dark shadow-[var(--shadow-md)]
                             flex items-center justify-between gap-5 border border-white/10"
                >
                  <EdgeFrame radius="rounded-xl" />
                  {/* corner sheen */}
                  <div
                    aria-hidden
                    className="absolute -top-px -left-px w-20 h-20 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.04),transparent_70%)] rounded-xl"
                  />
                  
                  {/* Content area: horizontal flex with icon on the left, label on the right */}
                  <div className="flex items-center gap-4">
                    {/* Icon */}
                    <span className="relative flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 border border-white/10 group-hover:border-white/25 transition-colors duration-300 flex-shrink-0">
                      <m.icon className="w-4.5 h-4.5 text-white/70" strokeWidth={1.5} aria-hidden />
                    </span>
                    
                    {/* Label */}
                    <div className="flex flex-col">
                      <p className="relative font-display uppercase tracking-[0.14em] text-[10px] text-white/50 leading-relaxed">
                        {m.label}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative flex items-baseline font-display font-semibold leading-none flex-shrink-0 text-white">
                    {m.prefix && (
                      <span className="text-xl mr-0.5 text-white/60">{m.prefix}</span>
                    )}
                    <span className="text-3xl sm:text-4xl text-white">
                      <Counter value={m.value} decimals={m.decimals} play={metricsInView} />
                    </span>
                    {m.suffix && (
                      <span className="text-lg ml-0.5 text-white/50">{m.suffix}</span>
                    )}
                  </div>
  ...
  return (
    <motion.button
      ...
      className="group/btn relative flex items-center justify-center w-8 h-8 rounded-full overflow-hidden
                 bg-white/5 border border-white/10 text-white/70 hover:text-white transition-colors duration-300"
    >
      {children}
    </motion.button>
  );
